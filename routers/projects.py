...
@router.get("/projects/data-sources")
async def get_projects_with_data_sources(request: Request):
    user_id = request.state.user_id
    user = await User.get(id=user_id)
    projects = await Project.filter(assigned_users=user).prefetch_related('file_data_sources')
    projects_with_sources = [
        {
            'id': project.id,
            'title': project.title,
            'data_sources': [{'id': ds.id, 'file_name': ds.file_name} for ds in project.file_data_sources]
        } for project in projects
    ]
    return {'projects': projects_with_sources}



@router.get("/projects/{id}/users")
async def get_project_users(id: int, request: Request):
    project = await Project.get(id=id).prefetch_related("assigned_users")

    # Retrieve all assigned users without checking for project creator
    users = [{"id": user.id, "email": user.email} for user in await project.assigned_users.all()]
    return {"users": users}
